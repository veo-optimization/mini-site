...
function parseClientConstants(constantsText) {
    ...
    lines.forEach(line => {
        ...
        // Використовуємо [\s\S]* замість .+ щоб захопити всі символи включно з переносами рядків
        const match = line.match(/^\s*c(\d+)([a-z]|_count|_type)?\s*-\s*([\s\S]*)$/);
        if (match) {
            const num = match[1];
            const suffix = match[2] || '';
            // НЕ використовуємо .trim() для значення, щоб зберегти пробіли на початку/кінці
            // Але видаляємо пробіли тільки на початку та в кінці рядка
            let value = match[3];
            // Видаляємо пробіли тільки на початку та в кінці, але зберігаємо всередині
            ...
            // Діагностика для c33
            if (key === 'c33') {
                console.log('📋 c33 знайдено, довжина:', value.length);
                console.log('📋 c33 містить "\\n" (текст):', value.includes('\\n'));
                console.log('📋 c33 містить "\n" (символ):', value.includes('\n'));
                console.log('📋 Перші 150 символів (JSON):', JSON.stringify(value.substring(0, 150)));
            }
        } else {
